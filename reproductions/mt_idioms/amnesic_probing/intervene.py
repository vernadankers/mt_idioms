...
def generate(model, tokenizer, sentence, projection=False, hidden_projection_indices=None,
    ...
    tgt_tokens = tokenizer.convert_ids_to_tokens(
        decoder_outputs["sequences"][0])
    return "".join(tgt_tokens[1:]).replace("▁", " ").strip(), attention


def main(language, data, hidden_projection_matrices,
    ...
    for j in sorted(list(data.keys())):
        ...
        for k, s in enumerate(data[j]):
            ...
            with_projection, attention_with = generate(
                ...
                hidden_projection_indices=s.index_select(
                    hidden_labels, no_subtokens=False),
            ...
            if gather_attention:
                ...
                noun_context_index = s.index_select(
                    0, tags=["NOUN"], neighbours_only=True)
                all_pie_index = s.index_select(1, tags=[])
                all_context_index = s.index_select(
                    0, tags=[], neighbours_only=True)
        ...
        idiom_scores[idiom] = 1 - \
            (idiom_equal_labels.count('paraphrase') / len(idiom_equal_labels))

    score = sacrebleu.corpus_bleu(changed_with, [changed_without]).score
    trace = {"src": all_src, "before": all_without,
             "after": all_with, "new_labels": all_new_labels}
    return score, idiom_scores, attention, trace
...
if __name__ == "__main__":
    ...
    parser.add_argument("--trace_filename", type=str,
                        default="data/trace.pickle")
    ...
    for fold in args.folds:
        logging.info(f"Starting with fold {fold}.")
        hidden_projection_matrices = []
        ...
        bleu, idiom_scores, attention, trace = main(
            args.language, {i: data[i] for i in test},

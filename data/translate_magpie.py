    ...
    with open(TEST_FILE_SRC, 'r', encoding="utf-8") as f_in, \
        ...
        for i in range(0, len(lines)+1, batch_size):
            if not lines[i:i+batch_size]:
                continue
            srcs, annotations = zip(*[x.split("\t")[:2]
                                    for x in lines[i:i+batch_size]])
            ...
            for j, src in enumerate(srcs):
                ...
                prd = tok.convert_ids_to_tokens(
                    outputs_beam["sequences"][j])[1:]
                attention[src] = np.array(
                    attention_tmp[j][:, :, :src_len, :src_len].cpu().detach().numpy(), dtype="half")
                hidden_states_enc[src] = hidden_states_enc_tmp[j].cpu(
                ).detach().transpose(0, 1).numpy()[:src_len].tolist()

                hidden_states_some_dropped = []
                # For tokens that do not belong to the idiom, only store 10%
                # to avoid filling up everyone hard drives with useless
                # but gigantic data
                for k, annotation in enumerate(src_annotations):
                    if k == 512:
                        break
                    if annotation == 1 or random.random() < 0.1 or args['language'] == "nl":
                        hidden_states_some_dropped.append(
                            hidden_states_enc[src][k])
                    else:
                        hidden_states_some_dropped.append(None)
                hidden_states_enc[src] = hidden_states_some_dropped
                cross_attention[src] = np.array(
                    cross_attention_tmp[j][:, :, :, :src_len].cpu().detach().numpy(), dtype="half")
                prds[srcs[j]] = prd
